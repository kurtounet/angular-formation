...
export class CbPipe implements PipeTransform {
  ...
  transform(value: any, num: number=4): string {
   ...
   // let end=`${value.slice(num, value.length)}`;
    return stars;
  }

  // transform(value: any, num: number = 4): unknown {
  //   let stars = ${value.slice(0, num)}${new Array(17-2*4).join('*')}${value.slice(-num, value.length)};
